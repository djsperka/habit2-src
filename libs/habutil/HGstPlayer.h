...
class HGstPlayer: public HPlayer
{
	Q_OBJECT
...
private:
    ...
    void handleStreamStatusMessage(const QGst::StreamStatusMessagePtr & message);
	...
	void setActivePad(void) { setActivePad(m_iPendingStimulusId); };
...
public:
	HGstPlayer(int ID = 0, QWidget* w = 0, const QDir& dir = QDir("/"));
	virtual ~HGstPlayer();
