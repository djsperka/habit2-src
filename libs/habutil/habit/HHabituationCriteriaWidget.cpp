...
namespace GUILib
{
	...
	HHabituationCriteriaWidget::HHabituationCriteriaWidget(int ntrials, const Habit::CriterionSettings& cs, int totalLookingTime, QWidget* w)
	: QStackedWidget(w)
	, ui(new Ui::HHabituationCriteriaWidget)
	{
		ui->setupUi(this);
	...
	int HHabituationCriteriaWidget::getTotalLookLength()
	{
		return ui->lineeditTotalLookingTime->text().toInt();
	}

	const Habit::CriterionSettings HHabituationCriteriaWidget::getCriterionSettings()
	{
