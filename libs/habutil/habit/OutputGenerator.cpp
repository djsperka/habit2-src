...
const int TrialLogItem::LEFT_LOOK = 0;
const int TrialLogItem::CENTER_LOOK = 1;
const int TrialLogItem::RIGHT_LOOK = 2;
const int TrialLogItem::AWAY_LOOK = 3;
const int TrialLogItem::NEW_TRIAL = 4;
const int TrialLogItem::NO_LOOK = 5;
const int TrialLogItem::REPEAT_TRIAL = 6;
const int TrialLogItem::NEW_TRIAL_REPEAT = 7;

void TrialInfo::addLogItem(int type, int duration, StimulusSettings settings)
{
	TrialLogItem item(type, duration, settings);
...
void OutputGenerator::setExperiment(int id) {
...
void OutputGenerator::setSubjectInformation(const SubjectSettings& si) {
	subjectInformation_ = si;
}

void OutputGenerator::setRunSettings(const RunSettings& rs) {
	runSettings_ = rs;
}

void OutputGenerator::setCurrentTrial(int st) {
	currentTrial_ = st;
}

void OutputGenerator::setStimulusSettings(const StimulusSettings& ss) {
	stimulusSettings_ = ss;
}

void OutputGenerator::setResultViewerSettings(const Habit::ResultViewerSettings &rvs)
{
	settings_ = rvs;
}

void OutputGenerator::changeTrial(int st) {
	if(currentTrial_ != st) {
...
void OutputGenerator::summarize() {
	switch(currentTrial_) {
		case StimulusSettings::HABITUATION :
			habituationInfo_ = currentTrialInfo_;
			break;
...
void OutputGenerator::addLogItem(int type, int duration)
{
	currentTrialInfo_.addLogItem(type, duration, stimulusSettings_);
}

QStringList OutputGenerator::getTrialSummary(const QString& phaseName)
{
	QStringList log;
...
QStringList OutputGenerator::getPhaseSummary(const QString& phaseName)
{
	QStringList log;
...
QStringList OutputGenerator::getPhaseLog(const QString& phaseName)
{
	QStringList log;
	...
	for (int i = 0; i < info.logItems.count(); i++)
	{
		...
		if (info.logItems[i].lookType == Habit::TrialLogItem::REPEAT_TRIAL)
		{
			log.append(QString("Trial %1 was repeated because no looking was recorded").arg(trialcount));
		}
		if (info.logItems[i].lookType == Habit::TrialLogItem::NEW_TRIAL)
		{
		...
		if (info.logItems[i].lookType == Habit::TrialLogItem::NEW_TRIAL_REPEAT)
		{
		...
		totalTime += duration;
		if (info.logItems[i].lookType == TrialLogItem::LEFT_LOOK)
		{
			leftTime += duration;
			leftLookCount++;
			allLookCount++;
		...
		if (info.logItems[i].lookType == TrialLogItem::CENTER_LOOK)
		{
			centerTime += duration;
			centerLookCount++;
			allLookCount++;
		...
		if (info.logItems[i].lookType == TrialLogItem::RIGHT_LOOK)
		{
			rightTime += duration;
			rightLookCount++;
			allLookCount++;
		...
		if (info.logItems[i].lookType == TrialLogItem::AWAY_LOOK)
		{
			//centerTime += duration;
			allLookAwayCount++;
			if (isShowTimeline)
		...
		if (info.logItems[i].lookType == TrialLogItem::NO_LOOK)
		{
			if (isShowTimeline)
...
QStringList OutputGenerator::getPhaseLookSummary(const QString& phaseName)
{
	QStringList log;
	log.append(QString("  %1 Phase:").arg(phaseName));
	TrialInfo info;
	...
	for (int i = 0; i < info.logItems.count(); i++)
	{
		...
		if (info.logItems[i].lookType == Habit::TrialLogItem::LEFT_LOOK)
		{
			++lookCount;
		...
		if (info.logItems[i].lookType == Habit::TrialLogItem::RIGHT_LOOK)
		{
			++lookCount;
		...
		if (info.logItems[i].lookType == Habit::TrialLogItem::CENTER_LOOK)
		{
			++lookCount;
		...
		if (info.logItems[i].lookType == Habit::TrialLogItem::NO_LOOK)
		{
			timeCount += duration;
		}
		if (info.logItems[i].lookType == Habit::TrialLogItem::AWAY_LOOK)
		{
			timeCount += duration;
		}
...
QStringList OutputGenerator::toHtml(bool headerIncluded) {
	QStringList log;
	if (headerIncluded)
	{
...
bool OutputGenerator::save(const QString &fileName)
{
	QFile file(fileName);
...
bool OutputGenerator::load(const QString &fileName)
{
	QFile file(fileName);
...
void OutputGenerator:: setExperimentName(const QString & name)
{
	experimentName_ = name;
}

QString OutputGenerator:: getExperimentName()
{
	return experimentName_;
}

void OutputGenerator:: setSettingsFileName(const QString & name)
{
	settingsFileName_ = name;
}

QString OutputGenerator:: getSettingsFileName()
{
	return settingsFileName_;
}

void OutputGenerator:: setResultsFileName(const QString & name)
{
	resultsFileName_ = name;
}

QString OutputGenerator:: getResultsFileName()
{
	return resultsFileName_;
}

void OutputGenerator:: setResultsType(const QString & name)
{
	resultsType_ = name;
}

QString OutputGenerator:: getResultsType()
{
	return resultsType_;
}

void OutputGenerator:: setTrialType(const QString & name)
{
	trialType_ = name;
}

QString OutputGenerator:: getTrialType()
{
	return trialType_;
}

void OutputGenerator:: setHabituation(const QString & value)
{
	habituation_ = value;
}

QString OutputGenerator:: getHabituation()
{
	return habituation_;
}

void OutputGenerator:: setHabituationResult(const QString & value)
{
	habituationResult_ = value;
}

QString OutputGenerator:: getHabituationResult()
{
	return habituationResult_;
}

bool OutputGenerator::isHabituated()
{
	summarize();
	...
	{
		TrialLogItem info = habituationInfo_.logItems[i];
		if (info.lookType == TrialLogItem::NEW_TRIAL)
		{
			if (i > 0)
			{
		...
		if (info.lookType == TrialLogItem::CENTER_LOOK ||
			info.lookType == TrialLogItem::LEFT_LOOK ||
			info.lookType == TrialLogItem::RIGHT_LOOK )
		{
