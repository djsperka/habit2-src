...
{
	dbg.nospace() << "ControlBarOptions: id " << cbo.getId() << " cbar used?  " << cbo.isControlBarUsed() << " expt displayed " << cbo.isCurrentExperimentDisplayed() << " stim displayed " << cbo.isCurrentStimulusDisplayed() << " looking dir displayed " << cbo.isLookingDirectionDisplayed() << Qt::endl;
	return dbg.nospace();
}
