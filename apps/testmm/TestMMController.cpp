...
, m_stringList(names)
{
	...
	QAbstractListModel *pmodel = new QStringListModel(m_stringList);
	...
	m_prerollButton = new QPushButton("Preroll");
	m_playButton = new QPushButton("Play");
	m_cleanupButton = new QPushButton("Cleanup");
////	m_replayButton = new QPushButton("Replay");
//
    buttonBox->addButton(m_prerollButton, QDialogButtonBox::ActionRole);
    buttonBox->addButton(m_playButton, QDialogButtonBox::ActionRole);
    buttonBox->addButton(m_cleanupButton, QDialogButtonBox::ActionRole);
//    buttonBox->addButton(m_stopButton, QDialogButtonBox::ActionRole);
//
    connect(m_playButton, SIGNAL(clicked()), this, SLOT(playClicked()));
    connect(m_prerollButton, SIGNAL(clicked()), this, SLOT(prerollClicked()));
    connect(m_cleanupButton, SIGNAL(clicked()), this, SLOT(cleanupClicked()));
...
void TestMMController::playClicked()
{
	...
	if (it.hasNext())
	{
		qDebug() << "selected row " << it.next().row();
...
void TestMMController::prerollClicked()
{
	QList<QModelIndex> selected = m_list->selectionModel()->selectedIndexes();
	QListIterator<QModelIndex> it(selected);
	qDebug() << "prerollClicked";
	if (it.hasNext())
	{
		qDebug() << "selected row " << it.next().row();
		// expected to have just 1
...
void TestMMController::cleanupClicked()
{
	QList<QModelIndex> selected = m_list->selectionModel()->selectedIndexes();
	QListIterator<QModelIndex> it(selected);
	qDebug() << "cleanupClicked";
	if (it.hasNext())
	{
		qDebug() << "selected row " << it.next().row();
		// expected to have just 1
